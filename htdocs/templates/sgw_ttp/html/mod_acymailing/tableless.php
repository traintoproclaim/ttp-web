...
<?php
	...
	<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		...
		<div class="acymailing_module_form row">
			<div class="col-md-6"><?php if(!empty($introText)) echo '<span class="acymailing_introtext">'.$introText.'</span>';?></div>
            ...
            if($oneField == 'name' AND empty($extraFields[$oneField])){
              if($displayOutside) echo '<label class="sr-only" for="user_name_'.$formName.'">'.$nameCaption.'</label>'; ?>
              ...
              if($displayOutside) echo '<label class="sr-only" for="user_email_'.$formName.'">'.$emailCaption.'</label>'; ?>
              ...
              echo '<label class="sr-only">'.JText::_('RECEIVE').'</label>';
        ...
        <?php }
         ...
         if($params->get('showterms',false)){
          echo '<p class="checkbox" id="field_terms_'.$formName.'">';
          ?>

          <label><input id="mailingdata_terms_<?php echo $formName; ?>" class="checkbox" type="checkbox" name="terms"/>
           <?php echo $termslink; ?></label>
          </p>
          <?php } ?>
            </div>
            <div class="col-md-4 col-xs-4 acysubbuttons">
              ...
              <input class="button unsubbutton btn btn-link" type="button" value="<?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?>" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"/>
              <?php } ?>
            </div>
          </div>
